...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('A062 BOT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('A062ID', 'yellow'))
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host && isWelcome) {
			await A062BOT.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
            await A062BOT.sendTextWithMentions(event.chat, `Hello, Welcome to the group @${event.who.replace('@c.us', '')} \n\nHave fun with us✨`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
			await A062BOT.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
            await A062BOT.sendTextWithMentions(event.chat, `Good bye @${event.who.replace('@c.us', '')}, We'll miss you✨`)
        }
    })

    A062BOT.onIncomingCall(async (callData) => {
        // ketika seseorang menelpon nomor bot akan mengirim pesan
        await A062BOT.sendText(callData.peerJid, 'Maaf sedang tidak bisa menerima panggilan.\n\n-bot')
        .then(async () => {
            // bot akan memblock nomor itu
            await A062BOT.contactBlock(callData.peerJid)
        })
    })

    // ketika seseorang mengirim pesan
    A062BOT.onMessage(async (message) => {
        A062BOT.getAmountOfLoadedMessages() // menghapus pesan cache jika sudah 3000 pesan.
            .then((msg) => {
                if (msg >= 3000) {
        ...
        HandleMsg(A062BOT, message)    
    
    })
	
    // Message log for analytic
    A062BOT.onAnyMessage((anal) => { 
